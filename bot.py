...
def get_examenes_keyboard(materia_key, tema_idx):
    ...
    examenes = unidad_keys and temario[unidad_keys[tema_idx]].get("examenes", [])
...
def get_pregunta_keyboard(opciones, materia, tema_idx, examen_idx, idx):
    ...
    for i,o in enumerate(opciones):
        mk.add(types.InlineKeyboardButton(o, callback_data=f"p:{materia}:{tema_idx}:{examen_idx}:{idx}:{i}"))
    mk.row(
...
def get_examen_fin_keyboard(state):
    ...
    mk.row(
        ...
        types.InlineKeyboardButton("🏠 Menú principal", callback_data="menu"),
        types.InlineKeyboardButton("❌ Cancelar / Salir", callback_data="salir_menu")
    )
    return mk
