...
def get_materias_keyboard():
    ...
    buttons = [types.InlineKeyboardButton(v['nombre'], callback_data=f"m:{i}") for i, v in enumerate(MATERIAS.values())]
    markup.add(*buttons)
...
def callbacks(call):
    ...
    if data.startswith('m:'):
        ...
        user_state[user_id].update({'materia_key': materia_key, 'tema_idx': tema_idx, 'examen_idx': examen_idx, 'preguntas': preguntas, 'pregunta_actual': 0, 'respuestas_correctas': 0})
...
def finalizar_examen(user_id, message_id):
    ...
    markup.row(
        types.InlineKeyboardButton('🔄 Repetir examen', callback_data=f'e:{state["materia"]}:{state["tema_idx"]}:{state["examen_idx"]}'),
